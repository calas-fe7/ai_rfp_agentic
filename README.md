...
- To use Azure OpenAI on your data, one of the following data sources:
  ...
  - Mongo DB (preview)
- For agentic retrieval (optional): Azure AI Search with knowledge agent configured
...
Agentic retrieval uses Azure AI Search knowledge agents to provide more intelligent search capabilities. The knowledge agent can analyze conversation history, break down complex queries, and provide enhanced retrieval results.

**Prerequisites:**
- Azure AI Search service with a knowledge agent configured
- Search index with semantic configuration and vector search enabled
- Azure OpenAI service with appropriate models deployed

**Configuration:**

| App Setting | Required? | Default Value | Note |
| --- | --- | --- | ------------- |
|AGENTIC_RETRIEVAL_ENABLED|No|False|Enable agentic retrieval functionality|
|AGENTIC_RETRIEVAL_SEARCH_ENDPOINT|Only if agentic retrieval is enabled||Azure AI Search endpoint (e.g., https://your-search-service.search.windows.net)|
|AGENTIC_RETRIEVAL_SEARCH_KEY|Optional if using managed identity||Azure AI Search API key|
|AGENTIC_RETRIEVAL_AGENT_NAME|Only if agentic retrieval is enabled|rfp-agent|Name of the knowledge agent in Azure AI Search|
|AGENTIC_RETRIEVAL_INDEX_NAME|Only if agentic retrieval is enabled||Name of the search index to use for retrieval|
|AGENTIC_RETRIEVAL_RERANKER_THRESHOLD|No|2.5|Reranker threshold for semantic ranking|
|AGENTIC_RETRIEVAL_SYSTEM_MESSAGE|No|"A Q&A agent that can answer questions about RFP's..."|System message for the knowledge agent|

For detailed setup instructions, see [AGENTIC_RETRIEVAL_README.md](./AGENTIC_RETRIEVAL_README.md).
