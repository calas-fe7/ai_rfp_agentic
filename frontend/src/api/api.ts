...
export const getAgenticRetrievalStatus = async (): Promise<Response> => {
  const response = await fetch('/agentic_retrieval/status', {
    method: 'GET',
    headers: {
      'Content-Type': 'application/json'
    }
    ...
    .catch(_err => {
      console.error('There was an issue getting agentic retrieval status.')
      const errRes: Response = {
        ...new Response(),
        ok: false,
        status: 500
