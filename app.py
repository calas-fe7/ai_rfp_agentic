...
frontend_settings = {
    ...
    "agentic_retrieval_enabled": (
        app_settings.agentic_retrieval and
...
def prepare_model_args(request_body, request_headers):
    ...
    # Add system message based on configuration
    if app_settings.agentic_retrieval and app_settings.agentic_retrieval.enabled:
        messages = [
            {
                "role": "system",
                "content": app_settings.agentic_retrieval.system_message
            }
        ]
    elif not app_settings.datasource:
        messages = [
            {
                "role": "system",
...
async def complete_chat_request(request_body, request_headers):
    if app_settings.base_settings.use_promptflow:
        ...
        # Check if agentic retrieval is enabled and available
        if (app_settings.agentic_retrieval and 
            app_settings.agentic_retrieval.enabled and 
            agentic_retrieval_service.is_available()):
            ...
            if retrieval_content:
                ...
                if messages and messages[-1]["role"] == "user":
                    # Create a new message with the retrieved context
                    enhanced_message = {
                        "role": "user",
                        "content": f"Based on the following retrieved information:\n\n{retrieval_content}\n\nPlease answer the user's question: {messages[-1]['content']}"
                    }
                    
                    # Replace the last message with the enhanced version
                    request_body["messages"][-1] = enhanced_message
...
@bp.route("/agentic_retrieval/status", methods=["GET"])
async def get_agentic_retrieval_status():
    """Get the status of agentic retrieval service"""
    try:
        if not app_settings.agentic_retrieval:
            return jsonify({
                "enabled": False,
                "status": "not_configured",
                "message": "Agentic retrieval is not configured"
            }), 200
        
        is_available = agentic_retrieval_service.is_available()
        
        return jsonify({
            "enabled": app_settings.agentic_retrieval.enabled,
            "available": is_available,
            "status": "available" if is_available else "unavailable",
            "agent_name": app_settings.agentic_retrieval.agent_name if app_settings.agentic_retrieval else None,
            "index_name": app_settings.agentic_retrieval.index_name if app_settings.agentic_retrieval else None,
            "message": "Agentic retrieval is available" if is_available else "Agentic retrieval is not available"
        }), 200
        
    except Exception as e:
        logging.exception("Exception in /agentic_retrieval/status")
        return jsonify({"error": str(e)}), 500
