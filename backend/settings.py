...
class _AzureSearchSettings(BaseSettings, DatasourcePayloadConstructor):
    ...
    use_agentic_retrieval: bool = Field(default=False, exclude=True)
    reranker_threshold: float = Field(default=2.5, exclude=True)
...
class _AgenticRetrievalSettings(BaseSettings):
    model_config = SettingsConfigDict(
        env_prefix="AGENTIC_RETRIEVAL_",
        env_file=DOTENV_PATH,
        extra="ignore",
        env_ignore_empty=True
    )
    ...
    agent_name: str = Field(default="rfp-agent")
    search_endpoint: str
    search_key: Optional[str] = None
    index_name: str
    reranker_threshold: float = Field(default=2.5)
    system_message: str = Field(
        default="A Q&A agent that can answer questions about RFP's (requests for proposals) for the Foundever company. If you do not have the answer, respond with 'I don't know'.",
        validation_alias="AGENTIC_RETRIEVAL_SYSTEM_MESSAGE"
    )
...
class _AppSettings(BaseModel):
    ...
    agentic_retrieval: Optional[_AgenticRetrievalSettings] = None
    ...
    @model_validator(mode="after")
    def set_agentic_retrieval_settings(self) -> Self:
        try:
            self.agentic_retrieval = _AgenticRetrievalSettings()
            logging.debug("Agentic retrieval configured")
        except ValidationError:
            self.agentic_retrieval = None
            logging.debug("Agentic retrieval not configured")
